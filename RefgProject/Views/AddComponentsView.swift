...
class AddComponentsView: UIView {
    ...
    let radioButton: DLRadioButton = {
        ...
        btn.titleLabel?.font = UIFont(name: "NotoSansKR-Thin_Regular", size: 18)
        btn.indicatorColor =  UIColor.getCustomColor()
        btn.iconColor =  .gray
        btn.isIconOnRight = true
        btn.marginWidth = -30
        return btn
    }()
    ...
    lazy var componentStackView: UIStackView = {
        ...
        stackView.addArrangedSubview(nameComponentLabel)
        stackView.addArrangedSubview(componentTextField)
        return stackView
    }()
...
//        tf.isUserInteractionEnabled = false
        textField.tag = 2
        textField.text = dateHelper.todayString
        return textField
    }()
    ...
    lazy var registerDateStackView: UIStackView = {
        ...
        stackView.addArrangedSubview(registerDateTextField)
    ...
    lazy var expirationDateTextField: CustomTextField = {
        let testField = CustomTextField()
//        tf.isUserInteractionEnabled = false
        testField.tag = 3
        testField.text = dateHelper.calculateDate(day: 7)
        return testField
    }()
