...
//        }
        ...
        //왼쪽패딩 --유일하게 동작하는 함수, 챗지피티는쓰지맙시다
        newConfig.contentInsets = NSDirectionalEdgeInsets(top: 0, leading: 2, bottom: 0, trailing: 0)
        
        //아래 코드는 동작안함 ㅡㅡ
//        newConfig.attributedTitle = AttributedString(title, attributes: AttributeContainer([NSAttributedString.Key.font : UIFont(name: "NotoSansKR-Thin_Regular", size: 40)!]))
    ...
    lazy var  switchStackView: UIStackView = {
        ...
        st.alignment = .fill
        st.addSubview(switchList)
        st.addSubview(switchLabel)
...
//        sv.addArrangedSubview(refViews[1])
    ...
    lazy var refStackView2: UIStackView = {
        ...
        sv.distribution = .fillEqually
        sv.spacing = 8
        //sv.addArrangedSubview(refViews[2])
      //  sv.addArrangedSubview(refViews[3])
        return sv
    }()
    ...
    lazy var refStackView3: UIStackView = {
      //  let sv = UIStackView(arrangedSubviews:[refStackView,refStackView2])
        let sv = UIStackView()
        sv.translatesAutoresizingMaskIntoConstraints = false
        sv.axis = .vertical
        sv.alignment = .fill
    ...
    let sortDeleteButton: UIButton = {
        ...
        btn.backgroundColor = .white
        btn.contentHorizontalAlignment = .right //오른쪽 정렬
        btn.titleLabel?.font = UIFont(name: "NotoSansKR-Thin_Regular", size: 14)
        return btn
    }()
    ...
    func createLabel(_ gesture: UIPanGestureRecognizer ,_ componentData: ComponentData) -> UIButton {
        let label: UIButton = {
            let v = UIButton()
            v.translatesAutoresizingMaskIntoConstraints = false
            v.setTitle(componentData.name, for: .normal)
            ...
            v.configuration = buttonConfig
            v.backgroundColor = UIColor(hexString: componentData.tagColor ?? "FF5D47")
...
//        ])
        ...
        //guard let refName = kindOfRef?.name else { return }
