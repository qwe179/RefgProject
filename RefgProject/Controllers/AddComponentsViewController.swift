...
class AddComponentsViewController: UIViewController {
    ...
    private var whichTextField: UITextField? = nil
...
//            return nil}
        ...
        return Component(refID: data.refID!, id: UUID().uuidString, name: name , registerDay: registerDay, dueDay: dueDay, isFreezer: isFreezer, memo: memo, tagColor: tagColor, coordinates: "")
    }
        ...
        if textField.tag == 2 || textField.tag == 3 {
            ...
            if let date = textField.text?.components(separatedBy: ".") {
                if let year = Int(date[0]), let month = Int(date[1]), let day = Int(date[2]) {
                    ...
                    selectionDate.selectedDate = DateComponents(calendar: Calendar(identifier: .gregorian), year: year, month: month, day:day)
