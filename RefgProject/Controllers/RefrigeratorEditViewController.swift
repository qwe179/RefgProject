...
class RefrigeratorEditViewController: UIViewController {
    ...
    func setCollectionView() {
        ...
        refrigeratorEditView.collectionView.register(
            RefgCollectionViewCell.self,
            forCellWithReuseIdentifier: "RefgCell"
        )
    }

    func setTargets() {
        refrigeratorEditView.saveButton.addTarget(self, action: #selector(saveButtonTapped), for: .touchUpInside)
        refrigeratorEditView.fridgeDeleteButton.addTarget(
            self,
            action: #selector(fridgeDeleteButtonTapped),
            for: .touchUpInside
        )
    }
    ...
    @objc func fridgeDeleteButtonTapped() {
        let confirmAlert = UIAlertController(
            title: nil,
            message: "냉장고가 하나일 때는 삭제할 수 없습니다",
...
extension RefrigeratorEditViewController: UICollectionViewDataSource,
